...
pc.script.create('PlayerControl', function(app) {
  ...
  makeLinearGradient = function() {
    ...
    var _glIndex = palletIndex / 8, glIndex = parseInt(_glIndex);
    if(_glIndex == glIndex || !pc.sclr) {
      // 是整数
      glIndex = Math.ceil(glIndex / 2);
      glIndex >= 11 && (glIndex = 10);
      var sclr = lg[glIndex][0],
          eclr = lg[glIndex][1];
    ...
    SpanCube: function(event) {
      ...
      if (this.gamemanger.lose) {
        ...
        if (this.anotherPosition.x != this.newPosition.x || this.anotherPosition.z != this.newPosition.z) {
          ...
          this.Count++;
          // Combo
          var _combo = pc.combo || 4; // 默认4个出烟花
          if(this.Count % pc.combo == 0){
          ...
          // 连击变大只需要改下面的数字即可
          var _perfect = pc.perfect || 4; // 默认4个
          if (this.Perfect >= _perfect && this.Perfect != this.score + 1) {
            if (!this.newYaw) {
